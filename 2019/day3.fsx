...
let getLine point (str: string) =
    let direction = str.[0]
    let endPoint = float str.[1..]
    if direction = 'R' then
        { XPos = endPoint + point.XPos
          YPos = point.YPos }
    else if direction = 'U' then
        { XPos = point.XPos
          YPos = endPoint + point.YPos }
    else if direction = 'D' then
        { XPos = point.XPos
          YPos = point.YPos - endPoint }
    else
        { XPos = point.XPos - endPoint
          YPos = point.YPos }

let rec coordinates wire prev (input: list<string>) =
    let newPoint = getLine prev (List.head input)
    if List.length input = 1 then
        List.append wire [ newPoint ]
    else
        let newWire = List.append wire [ newPoint ]
        coordinates newWire newPoint (List.tail input)

//From https://stackoverflow.com/questions/42289225/f-take-a-list-return-list-of-tuples
let rec getLines coords =
    match coords with
    | []
    | [ _ ] -> []
    | x1 :: x2 :: rest -> (x1, x2) :: (getLines (x2 :: rest))

//calcIntersection adapted from http://www.navision-blog.de/blog/2008/12/02/calculate-the-intersection-of-two-lines-in-fsharp-2d/
let calcIntersection (A: Location, B: Location, C: Location, D: Location) =
    let (Ax, Ay, Bx, By, Cx, Cy, Dx, Dy) = (A.XPos, A.YPos, B.XPos, B.YPos, C.XPos, C.YPos, D.XPos, D.YPos)
...
let rec buildIntersectionsLine res (line: Location * Location) (lines: list<Location * Location>) =
    let testLine = List.head lines
    let intersect = calcIntersection (fst line, snd line, fst testLine, snd testLine)
    if List.length lines = 1 then
        if intersect.XPos = 0.0 && intersect.YPos = 0.0 then res
        else List.append res [ intersect ]
    else
        let newRes = List.append res [ intersect ]
        if intersect.XPos = 0.0 && intersect.YPos = 0.0 then buildIntersectionsLine res line (List.tail lines)
        else buildIntersectionsLine newRes line (List.tail lines)

let buildIntersectionsList list1 list2 = list1 |> List.map (fun x -> buildIntersectionsLine [] x list2)
let manhattanDistance (x1, y1) (x2, y2) = abs (x1 - x2) + abs (y1 - y2)

let distances start inter =
    let results = inter |> List.map (fun x -> manhattanDistance (start.XPos, x.YPos) (x.XPos, start.YPos))
    List.min results
...
let lines1 = getLines coordinates1
...
let lines2 = getLines coordinates2

let x = buildIntersectionsList lines1 lines2
