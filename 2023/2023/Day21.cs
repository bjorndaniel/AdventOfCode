...
public class Day21
{
    ...
    //public static ((int x, int y) start, char[,] plot) ParseInput(string filename)
    //{
    //    var lines = File.ReadAllLines(filename);
    //    var plot = new char[lines.First().Length, lines.Length];
    //    var start = (0, 0);
    //    for (int row = 0; row < lines.Length; row++)
    //    {
    //        for (int col = 0; col < lines.First().Length; col++)
    //        {
    //            plot[col, row] = lines[row][col];
    //            if (lines[row][col] == 'S')
    //            {
    //                start = (col, row);
    //            }
    ...
    {
        var lines = File.ReadAllLines(filename);
        var plot = new Dictionary<(int x, int y), char>();//new char[lines.First().Length, lines.Length];
        var start = (0, 0);
        for (int row = 0; row < lines.Length; row++)
        {
            for (int col = 0; col < lines.First().Length; col++)
    ...
    public static SolutionResult Part2(string filename, IPrinter printer)
    {
        ...
        DFS(plot, start.x, start.y, filename.Contains("test") ? 50 : 64, plot, true, originalX, originalY);
        return new SolutionResult(_found.Count().ToString());
    }
    ...
    //private static long DFS(char[,] plot, int x, int y, int steps)
    //{
    //    if (x < 0 || x >= plot.GetLength(0) || y < 0 || y >= plot.GetLength(1) || plot[x, y] == '#')
    //    {
    //        return 0;
    //    }
    ...
    //        var newX = x + _directions[i].x;
    //        var newY = y + _directions[i].y;
    //        count += DFS(plot, newX, newY, steps - 1);
    //    }
